...
router.get('/:user_id', async (req, res) => {
    try {
        const profile = await Profile.findOne({ user: req.params.user_id }).populate('user', ['name', 'user'])
        if (!profile) return res.status(400).json({ msg: "Profile doesn't exist for this user" })
        res.json(profile);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
//desc: Delete education from profle
//access Private
